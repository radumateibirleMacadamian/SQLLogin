...
GO
PRINT N'Creating [dbo].[UserInfoSearch]...';


GO
CREATE VIEW [dbo].[UserInfoSearch]
	AS SELECT [User].ID, [User].DisplayName,[PersonalDetails].PhoneNumber, [Permission].Name FROM [User] INNER JOIN [PersonalDetails] ON [User].Details = [PersonalDetails].ID INNER JOIN [UserHasPermission] ON [User].ID = [UserHasPermission].UserID INNER JOIN [Permission] ON [UserHasPermission].PermissionID = [Permission].ID
GO
/*
